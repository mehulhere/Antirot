import { appendEvent, appendSleepEntry, nowIso, readSleepStats, todayKey, writeSleepStats } from "./storage.js";
...
export function dateAfterHours(date, hours) {
    return new Date(date.getTime() + hours * 60 * 60 * 1000);
}
function normalizeTiredness(value) {
    return clamp(Number.isFinite(value ?? 0) ? value ?? 0 : 0, 0, 3);
}
function rollingDebt(entries, fallbackDebtHours) {
...
export function calculateSleepRequirement(stats, tirednessLevel, plannedSleepHours) {
...
export async function getSleepSummary(workspaceDir, tirednessLevel) {
...
export function isGoodMorningVariant(text) {
