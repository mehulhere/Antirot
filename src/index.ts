...
function readString(params: ToolParams, key: string): string {
...
function readNumber(params: ToolParams, key: string): number {
    const value = params[key];
...
function readOptionalNumber(params: ToolParams, key: string): number | undefined {
    const value = params[key];
...
function readOptionalBoolean(params: ToolParams, key: string): boolean | undefined {
    const value = params[key];
    if (value === undefined || value === null) {
        return undefined;
    }
...
function applyPatch(content: string, patch: string): string {
...
function readBoolean(params: ToolParams, key: string): boolean {
    const value = params[key];
    if (typeof value !== "boolean") {
        throw new Error(`${key} must be true or false.`);
    }
...
function readOptionalStringArray(params: ToolParams, key: string): string[] | undefined {
    const value = params[key];
    if (value === undefined || value === null) {
        return undefined;
    }
...
export async function getOnboardingStatus(workspaceDir: string, state?: AntirotState): Promise<{
    missing: string[];
    reviewDue: boolean;
    nextQuestion: string;
}> {
...
function resolveWorkspace(api: OpenClawPluginApi, ctx?: OpenClawPluginToolContext | PluginCommandContext): string {
    const toolContext = ctx as OpenClawPluginToolContext | undefined;
    const commandContext = ctx as PluginCommandContext | undefined;
...
function resolveRuntimeConfig(api: OpenClawPluginApi): AntirotConfig {
    return asConfig(api.pluginConfig);
}

function eventWorkspace(api: OpenClawPluginApi): string {
    return resolveWorkspaceDir({ config: asConfig(api.pluginConfig), openClawConfig: api.config });
}

function today(statsDate = new Date()): string {
    return todayKey(statsDate);
}

export async function selectDailyStrategies(
    workspaceDir: string,
    state: AntirotState,
    config?: AntirotConfig
): Promise<AntirotState> {
    const day = today();
    ...
    const selected = new Set<string>(ranked.slice(0, bestCount));
...
async function logOverride(workspaceDir: string): Promise<number> {
...
async function toggleVacation(workspaceDir: string, enabled: boolean): Promise<AntirotState> {
    await ensureWorkspace(workspaceDir);
    const state = await readState(workspaceDir);
    const nextState: AntirotState = {
        ...state,
        vacation: enabled,
...
async function buildStateContext(workspaceDir: string, config: AntirotConfig): Promise<string> {
    await ensureWorkspace(workspaceDir);
