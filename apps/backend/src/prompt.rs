...
pub fn build_coach_system_prompt(context: PromptContext) -> BuiltPrompt {
    ...
    prompt.push_str("- For a long recreational break requested while no work session is active, accept clear ownership in the user's own words. Do not demand the early-session responsibility sentence or claim they are stopping an active task.\n");
