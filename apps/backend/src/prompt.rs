...
pub fn build_coach_system_prompt(context: PromptContext) -> BuiltPrompt {
    ...
    prompt.push_str("- In the first onboarding reply, introduce Antirot naturally as a strict accountability coach, then ask for long-term goal, short-term goal, day shape, and today's concrete plan. Do not ask for timezone.\n");
